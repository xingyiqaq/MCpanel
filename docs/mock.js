...
window.ENTITIES_LIST=[
  ...
  {id:'minecraft:wither_skeleton',name:'凋灵骷髅',cat:'亡灵'},
  ...
  {id:'minecraft:parrot',name:'鹦鹉',cat:'友好'},
  {id:'minecraft:axolotl',name:'美西螈',cat:'友好'},
  ...
  {id:'minecraft:fox',name:'狐狸',cat:'友好'},
  {id:'minecraft:bee',name:'蜜蜂',cat:'友好'},
  {id:'minecraft:guardian',name:'守卫者',cat:'敌对'},
  {id:'minecraft:evoker',name:'唤魔者',cat:'敌对'},
  {id:'minecraft:pillager',name:'流浪者',cat:'敌对'},
  {id:'minecraft:ravager',name:'劫兽',cat:'敌对'},
...
window.ENCHANT_LIST=[
  ...
  {id:'bane_of_arthropods',name:'节肢杀手',target:'武器'},
  ...
  {id:'projectile_protection',name:'弹射物保护',target:'盔甲'},
  ...
  {id:'punch',name:'击退',target:'弓'},
  {id:'flame',name:'火焰',target:'弓'},
...
window.MOCK_COMMANDS={
  ...
  commands:[
    ...
    {cmd:'/reload',cn:'重载配置',cat:'管理',desc:'重载服务器配置'},
...
var DEMO_LOGS=[
  ...
  '[16:30:01] [Server thread/INFO]: Default game type: SURVIVAL',
  '[16:30:02] [Server thread/INFO]: Generating keypair',
  ...
  '[16:30:15] [Server thread/INFO]: xingyi: /say 大家好！',
  '[16:30:15] [Server thread/INFO]: <xingyi> 大家好！',
  '[16:30:20] [Server thread/INFO]: Steve: /gamemode creative',
...
function getNextLog(){
  ...
  if(_logIdx>=DEMO_LOGS.length){
    ...
    logs.push('['+now+'] [Server thread/INFO]: There are '+Math.floor(Math.random()*4+2)+' players online: '+ONLINE_PLAYERS[Math.floor(Math.random()*ONLINE_PLAYERS.length)]);
  }
  return logs;
}
...
function mockCommandReply(cmd){
  ...
  if(c==='list'){return '✅ There are 6 players online: '+ONLINE_PLAYERS.join(', ')}
  if(c==='seed'){return '✅ Level seed: 382910472638291'}
  if(c==='kill'){return '✅ Removed 1 entity'}
  ...
  if(c==='kill @e[type=!player]'){return '✅ Removed 47 entities'}
  ...
  return new Promise(function(resolve,reject){
    ...
    try{
      if(url==='/api/auth/check'){
        data={authenticated:true,username:'admin',must_change:false};
        ...
        data={
          tps:19.8,
          playerCount:6,
          players:ONLINE_PLAYERS.slice(),
          memoryUsed:1073741824*Math.random()+536870912,
          timestamp:new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),
          pids:[Math.floor(Math.random()*9000+1000)]
        };
      }else if(url==='/api/serverinfo'){
        data={serverType:'vanilla',serverVersion:'1.21.4',mode:'生存',itemCount:ITEMS_LIST.length,entityCount:ENTITIES_LIST.length,optimizeMode:'auto'};
      }else if(url==='/api/items'){
        data={items:ITEMS_LIST.slice(),enchantments:ENCHANT_LIST.slice(),namespaces:[{namespace:'minecraft',name:'原版',count:ITEMS_LIST.length}]};
      }else if(url==='/api/entities'){
        data={entities:ENTITIES_LIST.slice(),namespaces:[{namespace:'minecraft',name:'原版',count:ENTITIES_LIST.length}]};
      }else if(url==='/api/log'){
        data={lines:getNextLog()};
      }else if(url==='/api/commands/cats'){
        data={categories:['','信息','玩家','世界','物品','实体','管理','技术']};
      }else if(url==='/api/commands'){
        // POST请求
        var body=opts&&opts.body?JSON.parse(opts.body):{};
        var kw=(body.kw||'').toLowerCase();
        var cat=body.category||'';
        var cmds=MOCK_COMMANDS.commands.slice();
        ...
        cmd='/'+cmdMap[type]||'/unknown '+JSON.stringify(params);
        data={command:cmd};
        ...
        data={raw:{},categories:[{id:'游戏性',items:[]},{id:'世界',items:[]},{id:'服务器',items:[]}]};
      }else if(url==='/api/config/save'){
        data={ok:true};
      }else if(url==='/api/panelconfig'){
        data={port:19888,name:'MC 服务端管理面板',mode:'auto',rcon_host:'127.0.0.1',rcon_port:25575,server_dir:'.'};
      }else if(url==='/api/panelconfig/save'){
        data={ok:true};
      }else if(url==='/api/auth'){
        data={ok:true};
      }
    }catch(e){
      data={error:String(e.message)};status=500;
    }
...
(function tpsLoop(){
  setInterval(function(){
    ...
    if(el){
      var tps=(19+Math.random()*0.9).toFixed(1);
      el.textContent=tps;
    }
    var memEl=document.getElementById('v-mem');
    if(memEl){
      var mem=(1.5+Math.random()*0.5).toFixed(1);
